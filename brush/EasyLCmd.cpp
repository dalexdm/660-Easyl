...
//try rendering
MStatus EasyLCmd::doIt( const MArgList& args )
{
		MStatus stat = MS::kSuccess;

		MGlobal::displayInfo(MString("Creating EasyL Node"));  
...
void EasyLCmd::Rendering(int Image_Height, int Image_Width, string name, int frameNum, int flag)
{
			//background color   
			...
			//correct the name for each EasyLNode   
			...
			for ( ; !itCo.isDone(); itCo.next() )   
			{
				...
				for(int w = NodeNum; w <= EasyLNode::numOfSplinesCreated; w++)
				{
					int result; 
					MString firstPart("objExists EasyLNode"); 
					MString cmd = firstPart + w; 
					MGlobal::executeCommand(cmd, result, true, true); 
					if(result == 1)
					{
						MString RNfirst ("rename ");
						MString rn = RNfirst + "EasyLNode" + w + " EasyLNode" + NodeNum; 
			...
			{   
				...
				MString firstPart ("getAttr EasyLNode"); 
		...
		if(flag == 2)
		{
			...
			// String path 
			string path = "C:\\CIS 660\\EasyLImage\\";
        ...
        {
                displayError( "EasyL: error occured in endRender." );
              //  return MS::kFailure;
        }  
}